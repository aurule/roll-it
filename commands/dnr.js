...
module.exports = {
  ...
  schema: Joi.object({
    ...
    pain: Joi.number().required().integer().min(1),
    exhaustion: Joi.number().integer().min(1).max(6)
      .when("talent", {
    ...
    talent: Joi.string().valid("minor", "major", "madness")
      .messages({
  ...
  perform({ discipline, pain, exhaustion, madness, talent, rolls, description, modifier }) {
    // add modifier to exhaustion
    console.log({ discipline, pain, exhaustion, madness, rolls, description })
  },
  execute(interaction) {
