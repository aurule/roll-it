...
module.exports = {
  ...
  rolls: Joi.number().optional().integer().min(1).max(100).messages({
  ...
  modifier: Joi.number().optional().integer().messages({
    "number.integer": "Modifier must be a whole number.",
  }),
  until: Joi.number().optional().integer().min(1).max(100),
  pool: Joi.number().required().integer().min(1).max(1000),
}
