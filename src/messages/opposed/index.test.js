...
import { messageIndex } from "./index.js"
...
describe("opposed messages", () => {
  it("loads message files", () => {
    expect(messageIndex.size).toBeGreaterThan(0)
  })

  it("indexes messages by state name", () => {
    expect(messageIndex.get("withdrawn").challengeState).toEqual("withdrawn")
  })

  it("excludes the index file", () => {
    expect(messageIndex.has(undefined)).toBeFalsy()
  })

  it.concurrent.each(
    messageIndex.map((m) => [m.challengeState, m]),
