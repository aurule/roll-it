...
export async function execute(interaction) {
  ...
  await interaction.message
    .edit(inertCancellingMessage(test.challenge_id, "cancel"))
    .catch((_error) => {
  ...
  if (test.leader) {
    ...
    next_message = winningMessage
  } else {
    next_state = Challenge.States.Tying
    next_message = tyingMessage
  }

  opposed_db.setChallengeState(test.challenge_id, next_state)

  return interaction
    .ensure("reply", next_message(test.challenge_id), {
      component: "opposed_cancel",
      test: test,
      detail: `failed to send ${next_state} prompt`,
    })
    .then((reply_result) => {
