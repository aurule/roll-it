import { ActionRowBuilder, userMention } from "discord.js"

import { sendMessage, editMessage } from "../services/api.js"
 ...
 * @type {number}
 */
export const MAX_DURATION = 900_000 // 15 minutes
 ...
 */
export async function teamworkBegin({ interaction, description, command, options, pool }) {
 ...
 */
export async function teamworkTimeout(teamwork_id) {
  const teamwork_db = new Teamwork()
 ...
 */
export async function cleanup(teamwork_id) {
  const teamwork_db = new Teamwork()
  const test = teamwork_db.detail(teamwork_id)
